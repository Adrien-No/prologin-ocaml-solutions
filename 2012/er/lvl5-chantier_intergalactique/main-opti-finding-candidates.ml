(* instead of re-building the list of candidates for each node of the tree, *)
(* we just add the new reachable node *)
...
let prim_alg nb_s g =
  ...
  let find_candidate candidates : int* int =

    List.fold_left (fun acc cand -> if snd cand < snd acc then cand else acc) (List.hd candidates) candidates

  in

  let rec loop candidates marked sum_road =
    if marked = nb_s then sum_road
    else
      begin
        let s, road' = find_candidate candidates in
        mark.(s) <- true;
        loop (g.(s) @ candidates |> List.filter (fun (s, _) -> not mark.(s))) (marked+1) (road'+sum_road)
      end
  in
  mark.(0) <- true;
  loop [0, 0] 0 0
