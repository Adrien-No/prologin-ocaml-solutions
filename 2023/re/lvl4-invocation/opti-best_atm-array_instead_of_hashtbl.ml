...
let rec trie_of_language (prof: int) (lang: char list list) : trie =
  ...
  | _ ->
    ...
    let childs =
      groups
      |> List.map (fun (c,l) -> c, (let t = trie_of_language (prof+1) l in nb_descendants := !nb_descendants + (get_descendants t); t))
      |> List.fast_sort (fun (c, _) (c', _) -> Stdlib.compare c c')
      |> Array.of_list
    in
...
let rec count_postfixs t prefix =
  ...
  | Node(x, b, l), c::prefix_suite ->
    ...
    | Some trie' ->
      count_postfixs (snd trie') prefix_suite
...
let solve nb_prefixs prefixs nb_vocabs vocabulaire =
  let t =
    ...
    (* |> language_of_letters *)
    |> lol'
    |> List.fast_sort Stdlib.compare (*TODO sort_uniq ? *)
    |> trie_of_language 0
  in
  prefixs
  |> map (count_postfixs t) (* TODO faut opti ici, par exemple si deux prefix ont des racines communes *)
