...
type trie = Nil | Node of int * bool * ( (char * trie) array) (* nb_descendants, is_suffix, (character, childs) *)
...
let rec trie_of_language (lang: char list list) : trie =
  ...
  | _ ->
    ...
    let childs = List.map (fun (c,l) -> c, trie_of_language l) groups |> List.sort_uniq (fun (c, _) (c', _) -> Stdlib.compare c c') |> List.to_seq |> Array.of_seq in (* TODO peut etre qu'on fait bcp de tris *)
    let nb_descendants = Array.fold_left (fun acc (src, dst) -> get_descendants dst + acc) 0 childs in (* TODO can optimize *)
...
let rec count_postfixs t prefix =
  ...
  | Node(x, b, l), c::prefix_suite ->
    (* Array.iter (fun (c, _)-> Printf.printf "%c " c) l; print_newline(); *)
    (* printf "c= %c\n" c; *)
    (* TODO améliorable avec dichotomie *)
    (* match Array.find_opt (fun (key, value) -> key = c) l with *)
    match find_dicho c l with
    (* match Hashtbl.find_opt l c with *)
      None -> 0 (* failwith "count_postfixs: pas trouvé" *)
    | Some trie' ->
      count_postfixs (snd trie') prefix_suite (* TODO par sûr du snd *)
