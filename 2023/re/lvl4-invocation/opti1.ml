...
let rec trie_of_language (prof: int) (lang: char list list) : trie =
  ...
  | _ ->
    ...
    let h_childs = Hashtbl.create 32 in
    List.iter (fun (c,l) -> Hashtbl.add h_childs c (let t = trie_of_language (prof+1) l in nb_descendants := !nb_descendants + (get_descendants t); t)
                          ) groups;


    (* let nb_descendants = Hashtbl.fold (fun src dst acc -> get_descendants dst + acc) childs 0 in (\* TODO can optimize *\) *)
    Node(!nb_descendants, is_suffix, h_childs)
