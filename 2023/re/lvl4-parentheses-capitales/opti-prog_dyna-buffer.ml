...
let prog_valide (prog: string) : bool =
  ...
  (* let ss_strings () : char list list = *)

  (* let update_ss_progs () = *)
  (*   let b = Buffer.create 128 in *)
  (*   Buffer.add_string b prog; *)
  (*   for i = 0 to len do *)
  (*     for j = i to len do *)
  (*       ss_progs.(i).(j) <- Buffer.sub b i (j-i) *)
  (*     done *)
  (*   done *)
  (* in *)
  ...
  let rec dp prof a b =
    ...
    let add_to_cache =
             ...
             if i > b then false else
               let rec loop' j =
                 ...
                 if j > b then false
                   ...
                   match Hashtbl.find_opt lpars (String.sub prog a (i-a)) with
                   | None -> false
                   | Some i_rpar ->
                    loop' (j+1) || (rpars.(i_rpar) = (String.sub prog j (b-j)) && dp (prof+1) i j)
      ...
      Printf.sprintf "%sdp(%i, %i) = %b ; %s\n" (decalage prof) a b add_to_cache (String.sub prog a (b-a)) |> debug2;
      add_to_cache
    in
    "\n"|> debug2;
    dp 0 0 len
