...
let prog_valide (prog: string) : bool =
  ...
  let rec dp prof a b =
    ...
    let add_to_cache =
             ...
             if i > b then false else
               let rec loop' j =
                 ...
                 if j > b then false
                   ...
                   match Hashtbl.find_opt lpars (sub_string prog a i) with
                   | None -> false
                   | Some i_rpar ->
                    loop' (j+1) || (rpars.(i_rpar) = sub_string prog j b && dp (prof+1) i j)
      ...
      Printf.sprintf "%sdp(%i, %i) = %b ; %s\n" (decalage prof) a b add_to_cache (sub_string prog a b) |> debug2;
      add_to_cache
    in
    "\n"|> debug2;
    dp 0 0 len
